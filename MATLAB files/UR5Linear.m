...
function self = UR5Linear(useGripper)
...
%     end
    ...
    self.model.base = self.model.base * transl(-0.75,-0.3,0.25) * trotx(pi/2);% * troty(pi/2);
end
%% PlotAndColourRobot
...
function PlotAndColourRobot(self)%robot,workspace)
    ...
    % Display robot
    self.model.plot3d([-0.3 0 -pi/4 pi/4 0 -pi/2 0],'noarrow','workspace',self.workspace);
...
%%
function Move(self, qM)
            for i = 1:size(qM,1)
                p = qM(i,:);
                self.model.animate(p);
                drawnow();
            end
        end
    end
